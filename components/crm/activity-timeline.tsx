...
import { ActivityBadge, EmptyState } from "@/components/crm/ui";
import { formatEtDateTime } from "@/lib/crm/et";
import { refName, type ActivityWithRefs } from "@/lib/crm/joined";
        ...
        <li key={activity.id} className="py-4 first:pt-0 last:pb-0">
          ...
          {showRefs && (activity.contacts || activity.companies) ? (
            <p className="mt-2 text-xs text-muted">
              {activity.contacts ? (
                <Link
                  href={`/crm/contacts/${activity.contacts.id}`}
              ...
              {activity.companies ? (
                <Link
                  href={`/crm/companies/${activity.companies.id}`}
                  className="text-gold transition-colors hover:text-gold-hover"
                >
