...
export default function Discovery() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(NOTIFY_ENDPOINT, {
        ...
        body: JSON.stringify({
          _subject: `New Inquiry · ${fullName}${propertyName ? ` — ${propertyName}` : ""}`,
          _template: "table",
          _captcha: "false",
          _replyto: email,
          ...
          "Preferred date": consultDate ?? "No preference given",
          "Message": message ?? "—",
  ...
  return (
    <section id="contact-us" className="section-divider py-24 md:py-32">
